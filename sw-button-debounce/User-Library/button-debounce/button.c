...
bool button_raw(button_t *context){
...
bool button_read(button_t *context){
  ...
  if(context->state && ( (context->time - context->lastchange) >= context->on_delay )){
    context->fstate = true;
  }
  else if(!context->state && ( (context->time - context->lastchange) >= context->off_delay )){
